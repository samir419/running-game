        ...
        for (let i = 0; i < pattern.data.length; i++) {
            ...
            for (let j = 0; j < pattern.data[i].length; j++) {
                ...
                if (pattern.data[i][j] == 1) {
                    obj = {
                        x: j * this.obstacle_width + offset, y: 1080 - this.obstacle_height * rise, w: this.obstacle_width, h: this.obstacle_height, type: 'normal',
                ...
                if (pattern.data[i][j] == 2) {
                    obj = {
                        x: j * this.obstacle_width + offset, y: 1080 - this.obstacle_height * rise, w: this.obstacle_width, h: this.obstacle_height, type: 'bouncer',
                        oncollide: function (game, dir) {
                            if (dir == 'top') {
                                game.player.dy = -40
                            }
                ...
                if (pattern.data[i][j] == 3) {
                    obj = {
                        x: j * this.obstacle_width + offset, y: 1080 - this.obstacle_height * rise, w: 40, h: 25, type: 'missle',
                        oncollide: function (game, dir) {
                ...
                if (pattern.data[i][j] == 4) {
                    obj = {
                        x: j * this.obstacle_width + offset, y: 1080 - this.obstacle_height * rise, w: 100, h: 50, type: 'flying platform',
                        oncollide: function (game, dir) {
                            return
                        },
                        onupdate: function (self) {
                ...
                if (pattern.data[i][j] == 5) {
                    obj = {
                        x: j * this.obstacle_width + offset, y: 1080 - this.obstacle_height * rise, w: 100, h: 50, type: 'murder drone',
                        oncollide: function (game, dir) {
                            game.event_handler.publish('game over')
                        },
                        onupdate: function (self) {

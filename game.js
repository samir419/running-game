...
class Game {
    constructor(canvas, stage_data, character) {
        ...
        this.coin_count = 0
        this.score = 0
        this.highscore = parseInt(sessionStorage.getItem('running game highscore'), 10) || 0
        this.coin_img = new Image()
        this.coin_img.src = 'coin.png'
        this.set_player()
    }
    set_player() {
    ...
    render() {
        ...
        this.coins.forEach(coin => {
            if (!coin.collected) {
                ...
                //ctx.drawImage(this.coin_img, coin.x, coin.y, coin.w, coin.h)
            }
        })
    ...
    run() {
        if (this.state == 'running') {
            ...
            document.getElementById('high-score').textContent = `highscore: ${this.highscore}`
    ...
    handle_event(e) {
        if (e == 'game over') {
            ...
            if (this.score >= this.highscore) {
                ...
                sessionStorage.setItem('running game highscore', this.highscore)
            }
            this.state = 'pause'
